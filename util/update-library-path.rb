...
if path.empty?
  puts "ERROR: Specify path to apibuilder executable"
  exit(1)
end

if lib.empty?
  puts "ERROR: Specify path to apibuilder library"
  exit(1)
end
...
IO.readlines(path).each do |l|
  if l.strip == "load File.join(File.dirname(__FILE__), '../src/apibuilder-cli.rb')"
